...
const DEFAULT_LLM_MODEL = 'Qwen-Qwen2.5-7B-Instruct-GGUF';
const SIMILARITY_THRESHOLD = 0.8;
const MIN_MATCHES = 3;
const GENERATION_TIMEOUT = 180000; // 180 seconds (3 minutes) - give larger headroom for slow models

const normalizeGenerationMode = (mode) =>
  mode === 'manual' ? 'manual' : 'materials';
...
const buildDetailedPrompt = (projectName, userPrompt, generationMode) => {
  if (generationMode === 'manual') {
    return `Generate detailed instructions for this project: ${projectName}. Use this manual user brief as the source context: ${userPrompt}`;
  }

  return `Generate detailed instructions for this project: ${projectName}. Materials available: ${userPrompt}`;
};
 ...
 */
async function generatePhase1Names(userPrompt, modelName) {
  try {
 ...
 */
async function generateFullProjectDetails(
  projectName,
  userPrompt,
  materials,
  embedding,
  ...
  if (!project) {
    ...
    project.generationMode = generationMode;
    project.generationModel = modelName;
    await project.save();
  }
  ...
  try {
      ...
      response = await axios.post(
        LLM_API_URL,
        {
          ...
          messages: [
            ...
            {
              role: 'user',
              content: buildDetailedPrompt(projectName, userPrompt, generationMode),
            },
          ],
          temperature: 0.2,
    ...
    if (retryCount < MAX_RETRIES) {
      ...
      return generateFullProjectDetails(
        projectName,
        userPrompt,
        materials,
        embedding,
...
router.post('/', async (req, res) => {
  ...
  const modelName = normalizeModelName(req.body?.model);
  const userPrompt = generationMode === 'manual'
    ? req.body?.manualPrompt
    : req.body?.materials;
    ...
    return res.status(400).json({
      error:
        generationMode === 'manual'
          ? 'Please provide a manual prompt in the request body'
          : 'Please provide a materials string in the request body',
    });
  }
  ...
  try {
    ...
    const shouldUseCache = generationMode === 'materials';
    const cached = shouldUseCache
      ? await InputPromptCache.findOne({ prompt: userPrompt.trim() })
      : null;
    ...
    if (generationMode === 'materials' && materials.length === 0) {
      return res.status(400).json({
    ...
    const embedding = materials.length > 0
      ? await getMaterialsEmbedding(materials)
      : [];

    // Step 4: Search database for similar projects
    const dbMatches =
      generationMode === 'materials' && embedding.length > 0
        ? await searchSimilarProjects(materials, embedding)
        : [];
    ...
    if (dbMatches.length >= MIN_MATCHES && dbMatches[0].similarityScore >= SIMILARITY_THRESHOLD) {
      ...
      const phase1Results = await generatePhase1Names(userPrompt, modelName);
  ...
  try {
    ...
    await Project.findByIdAndUpdate(project._id, {
      $set: {
        ...
        generationModel: normalizeModelName(project.generationModel),
      },
    });

    if (wait) {
      try {
        const updated = await generateFullProjectDetails(
          project.projectName,
          project.inputPrompt || '',
          materials,
          embedding,
          ...
          {
            generationMode: normalizeGenerationMode(project.generationMode),
            modelName: normalizeModelName(project.generationModel),
          }
        );
