...
const Dashboard = () => {
  ...
  const [generationMode, setGenerationMode] = useState('materials');
  const [selectedModel, setSelectedModel] = useState('Qwen-Qwen2.5-7B-Instruct-GGUF');
  ...
  const applyPromptToActiveStudio = (prompt) => {
    if (generationMode === 'manual') {
      setManualPrompt(prompt);
      return;
    }

    setMaterialsInput(prompt);
  };

  const generateProjects = async () => {
    const activePrompt =
      generationMode === 'manual' ? manualPrompt.trim() : materialsInput.trim();

    if (!activePrompt) {
      setGenerateError(
        generationMode === 'manual'
          ? 'Please add a manual brief for the AI studio.'
          : 'Please describe the materials you have.'
      );
      return;
    }

    if (selectedModel === '__custom__' && !resolvedModel) {
      setGenerateError('Please enter your custom model name.');
      return;
    }
    ...
    try {
      const response = await axios.post(`${API_URL}/generate`, {
        generationMode,
        model: resolvedModel,
        materials: generationMode === 'materials' ? activePrompt : '',
        manualPrompt: generationMode === 'manual' ? activePrompt : '',
      });
      ...
      <div className="dashboard-container">
        ...
        <div className="dashboard-grid">
          ...
          <div id='generate-section' className="dashboard-section">
            ...
            <p>
              Run your DIY ideation through Qwen, switch between material-based
              generation and manual feed, then save the strongest result.
            </p>
            <div className="materials-input-group">
              ...
              <div className="studio-toolbar">
                <div className="studio-toggle-group">
                  <button
                    type="button"
                    className={`studio-toggle ${generationMode === 'materials' ? 'active' : ''}`}
                    onClick={() => setGenerationMode('materials')}
                  >
                    Material Mode
                  </button>
                  <button
                    type="button"
                    className={`studio-toggle ${generationMode === 'manual' ? 'active' : ''}`}
                ...
                <div className="studio-models">
                  <label htmlFor="modelSelect" className="studio-model-label">
                    AI model
                  </label>
                  <select
                    id="modelSelect"
                  ...
                  >
                    <option value="Qwen-Qwen2.5-7B-Instruct-GGUF">
                      Qwen 2.5 7B
                    </option>
              ...
              {selectedModel === '__custom__' && (
                <input
                  type="text"
                  className="studio-custom-model"
                  value={customModel}
                  onChange={(e) => setCustomModel(e.target.value)}
                  placeholder="Enter LM Studio model id"
                />
              )}
              <p className="studio-note">
                {generationMode === 'manual'
                  ? `Manual feed sends your full brief directly to ${resolvedModel || 'the selected model'} without restricting it to a materials-only prompt.`
                  : `Material mode uses ${resolvedModel || 'the selected model'} and still searches reusable project matches before generation.`}
              </p>
              {generationMode === 'materials' ? (
                  ...
                  <div className="suggestions-row">
                    {materialSuggestions.map((suggestion) => (
                      <button
                        ...
                        onClick={() => applyPromptToActiveStudio(suggestion)}
                      >
                        {suggestion}
                      </button>
                    ))}
                ...
                <textarea
                  ...
                  placeholder="Example: Create a compact balcony herb-garden project using recycled bottles, include kid-safe steps, a short budget, and a clean visual style."
                />
              )}
                    ...
                    {recentPrompts.map((p) => (
                      <div key={p.id} style={{ display: 'inline-flex', alignItems: 'center', marginRight: '6px' }}>
                        <button
                          type="button"
                          className="suggestion-chip"
                          onClick={() => applyPromptToActiveStudio(p.prompt)}
            ...
            <div className="generate-actions">
              <p>
                {generationMode === 'manual'
                  ? 'Manual feed is best when you already know the outcome you want and need the model to follow a richer creative brief.'
                  : 'Material mode first checks reusable DIY matches, then asks the AI if you need fresh inspiration.'}
              </p>
              <button
